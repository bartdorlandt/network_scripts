...
if { $VzB == "0" } {
  if { "$SSHLOG" == "yes" } {
    spawn script -q -c "ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no $IPaddress -l $Username" ${LOGDIR}/${IPaddress}_${DATE}.log
  } else {
#  #puts "vzb = $VzB"
    spawn ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no $IPaddress -l $Username
  }
} else {
